...
int sign_and_verify() {
	...
	OQS_STATUS rc;
        ...
        uint8_t public_key[OQS_SIG_sphincs_shake_128f_simple_length_public_key];
        uint8_t secret_key[OQS_SIG_sphincs_shake_128f_simple_length_secret_key];
        uint8_t message[MESSAGE_LEN];
        uint8_t signature[OQS_SIG_sphincs_shake_128f_simple_length_signature];
        size_t message_len = MESSAGE_LEN;
        size_t signature_len;

        OQS_randombytes(message, message_len);

        rc = OQS_SIG_sphincs_shake_128f_simple_keypair(public_key, secret_key);
        if (rc != OQS_SUCCESS) {
       		exit(EXIT_FAILURE);
       	}
	auto start = std::chrono::high_resolution_clock::now();

	rc = OQS_SIG_sphincs_shake_128f_simple_sign(signature, &signature_len, message, message_len, secret_key);
	...
	rc = OQS_SIG_sphincs_shake_128f_simple_verify(message, message_len, signature, signature_len, public_key);
